...
just_trait <- trait[, c(63,64,65,66,67,68,69,70,71,72,73)]
head(just_trait)
res <- cor(just_trait)
round(res, 2)
...
trait_z <- trait_z %>% 
  select(Species,MODERN, HISTORICAL,MIR_H_corr,MIR_M_LOST,MIR_EXT,MIR_NEW, BARB_H_corr,BARB_M_LOST,BARB_EXT,BARB_NEW,SAGU_H_corr, SAGU_M_LOST,SAGU_EXT,SAGU_NEW, TOCHE_H_corr,TOCHE_M_LOST,TOCHE_EXT, TOCHE_NEW,HON_H_corr,HON_M_LOST,HON_EXT, HON_NEW,FLOR_H_corr, FLOR_M_LOST,FLOR_EXT, FLOR_NEW, FUSA_H_corr, FUSA_M_LOST,FUSA_EXT, FUSA_NEW,weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) 
...
PCA_MIR <- trait_z %>% 
  filter(MIR_H_corr >= 0) %>% 
  select(Species,MIR_H_corr, MIR_M_LOST,MIR_EXT,MIR_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>% 
  left_join(scoresPCATotal_MIR, by = "Species") 

PCA_BARB <- trait_z %>% 
  filter(BARB_H_corr >= 0) %>% 
  select(Species,BARB_H_corr, BARB_M_LOST,BARB_EXT,BARB_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>%  
  left_join(scoresPCATotal_BARB, by = "Species") 

PCA_SAGU <- trait_z %>% 
  filter(SAGU_H_corr >= 0) %>% 
  select(Species,SAGU_H_corr, SAGU_M_LOST,SAGU_EXT,SAGU_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>% 
  left_join(scoresPCATotal_SAGU, by = "Species") 

PCA_TOCHE <- trait_z %>% 
  filter(TOCHE_H_corr >= 0) %>% 
  select(Species,TOCHE_H_corr, TOCHE_M_LOST,TOCHE_EXT,TOCHE_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>% 
  left_join(scoresPCATotal_TOCHE, by = "Species") 

PCA_HON <- trait_z %>% 
  filter(HON_H_corr >= 0) %>% 
  select(Species,HON_H_corr,HON_M_LOST,HON_EXT, HON_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>% 
  left_join(scoresPCATotal_HON, by = "Species") 

PCA_FLOR <- trait_z %>% 
  filter(FLOR_H_corr >= 0) %>% 
  select(Species,FLOR_H_corr, FLOR_M_LOST,FLOR_EXT,FLOR_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>% 
  left_join(scoresPCATotal_FLOR, by = "Species") 

PCA_FUSA <- trait_z %>% 
  filter(FUSA_H_corr >= 0) %>% 
  select(Species,FUSA_H_corr,FUSA_M_LOST,FUSA_EXT,FUSA_NEW, weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z) %>% 
  left_join(scoresPCATotal_FUSA, by = "Species") 
...
p = p + geom_density(alpha = 0.6) + scale_fill_manual(name = "POP_MIR", values=c("red", "azure3")) 
p = p + theme_bw(20) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.text=element_text(size=20),axis.title=element_text(size=20), axis.title.x = element_text(margin = unit(c(5, 0, 0, 0), "mm")))
p <- p + xlab("PC2 - Bill length (6%)") + ylab("Trait probability density") + ggtitle(" ")
p = p + theme(legend.position = "none")+ 
  scale_fill_manual(values=c("red", "azure3"), 
                    name=NULL,breaks=c("A", "C"),labels=c("Extirpated species", "Shared species"))
...
p = p + geom_density(alpha = 0.6) + scale_fill_manual(name = "POP_BARB", values=c("red", "azure3")) 
p = p + theme_bw(20) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.text=element_text(size=20),axis.title=element_text(size=20), axis.title.x = element_text(margin = unit(c(5, 0, 0, 0), "mm")))
p <- p + xlab("PC2 - Bill length (7%)") + ylab("Trait probability density") + ggtitle(" ")
p = p + theme(legend.position = "none")+ 
  scale_fill_manual(values=c("red", "azure3"), 
                    name=NULL,breaks=c("A", "C"),labels=c("Extirpated species", "Shared species"))
...
#subset scaled measures
trait_z <- trait_z %>% 
  select(Species,MODERN, HISTORICAL,MIR_EXT,BARB_EXT,SAGU_EXT,TOCHE_EXT, HON_EXT, FLOR_EXT, FUSA_EXT,weight_z,T_cul_z, Bill_w_z, Wing_l_z,Tail_l_z, Tars_z, HWI_z, EXTINCT) 
...
pca_plot_TOTAL <- ggplot(dcc_TOTAL, aes(x = Var1, y = Var2)) +
  ...
  theme(panel.grid.major = element_blank(),
        ...
        legend.position = "none",
        text = element_text(size = 22)) + 
  geom_text(x= 5, y=8, label="Extirpated and possibly extirpated species", color="black", size = 5)
...
pca_plot_TOTAL <- ggplot(dcc_TOTAL, aes(x = Var1, y = Var2)) +
  ...
  # axis labels - see comp_var
  labs(x = "PC2 - Dispersal ability (19%)", y = "PC3 - Bill morphology (6%)") +
  xlim(-8,18) +
  ylim(-8,8) +
  # edit plot
  theme(panel.grid.major = element_blank(),
